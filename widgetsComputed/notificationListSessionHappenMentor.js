...
sessionRows = $getGrid('sessions').filter(row => row.owner == fbUser.uid && row.isBooked == true && row.direction == "Mentor->CompanyUser" && row.rejected != true && row.sessionHappen != true)  
...
for (let i = 0; i < sessionRows.length; i++){
    ...
    if(sessionRowsDatesDifference[i] >= 3600000){
        $setDataGridVal("sessions", sessionRows[i].rowKey + ".sessionHappenNotifMentor", true);
    }
}
...
sessionHappenNotifRows = $getGrid('sessions').filter(row => row.owner == fbUser.uid && row.mentor == $getUser('profileID') && row.isBooked == true && row.direction == "Mentor->CompanyUser" && row.sessionHappen != true && row.rejected != true && row.sessionHappenNotifMentor == true)  
